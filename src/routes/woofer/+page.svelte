<script lang="ts">
  ...
  let downloadUrl = $state('https://github.com/minseokk77/vesper-woofer/releases/latest');
  let version = $state('');
...
<svelte:head>
  <title>Vesper Woofer - {i18n.t.wooferDetail.title}</title>
</svelte:head>

<section class="pt-32 pb-24 px-6 relative z-10 max-w-4xl mx-auto space-y-16">
  <!-- Header -->
  <div class="text-center space-y-4">
    <h1 class="text-5xl md:text-7xl font-semibold tracking-tighter text-[#f5f5f7]">Vesper Woofer</h1>
    <p class="text-xl text-[#86868b] tracking-tight">{i18n.t.wooferDetail.title}</p>
  </div>

  <!-- Stats Row -->
  <div class="grid grid-cols-3 gap-4">
    <div class="p-6 rounded-[24px] bg-white/[0.02] border border-white/[0.08] backdrop-blur-[60px] text-center">
      <div class="text-3xl font-bold text-violet-400 mb-1">{totalDownloads > 0 ? totalDownloads.toLocaleString() : '—'}</div>
      <div class="text-xs text-[#86868b]">{i18n.t.wooferDetail.totalDownloads}</div>
    </div>
    <div class="p-6 rounded-[24px] bg-white/[0.02] border border-white/[0.08] backdrop-blur-[60px] text-center">
  ...
  <div class="grid md:grid-cols-2 gap-6">
    <div class="p-10 rounded-[32px] bg-white/[0.02] border border-white/[0.08] shadow-[inset_0_1px_1px_rgba(255,255,255,0.1),0_8px_32px_rgba(0,0,0,0.4)] backdrop-blur-[60px]">
      <h3 class="text-2xl font-semibold tracking-tight text-[#f5f5f7] mb-4">{i18n.t.wooferDetail.box1Title}</h3>
      <p class="text-[#86868b] leading-relaxed font-medium text-[17px]">{i18n.t.wooferDetail.box1Desc}</p>
    </div>
    <div class="p-10 rounded-[32px] bg-white/[0.02] border border-white/[0.08] shadow-[inset_0_1px_1px_rgba(255,255,255,0.1),0_8px_32px_rgba(0,0,0,0.4)] backdrop-blur-[60px]">
      <h3 class="text-2xl font-semibold tracking-tight text-[#f5f5f7] mb-4">{i18n.t.wooferDetail.box2Title}</h3>
      <p class="text-[#86868b] leading-relaxed font-medium text-[17px]">{i18n.t.wooferDetail.box2Desc}</p>
    </div>
  </div>

  <!-- System Requirements -->
  <div class="p-8 rounded-[32px] bg-white/[0.02] border border-white/[0.08] backdrop-blur-[60px]">
    <h3 class="text-xl font-semibold text-[#f5f5f7] mb-6">{i18n.t.wooferDetail.sysReqTitle}</h3>
    <div class="flex flex-wrap gap-3">
      {#each [['OS', 'Windows 10/11'], ['Architecture', 'x64 (64-bit)'], ['RAM', '2GB+'], ['Runtime', '.NET 없음 (독립 실행형)'], ['Language', 'Rust']] as [label, value]}
  ...
  <!-- Changelog -->
  <div class="space-y-4">
    <h3 class="text-xl font-semibold text-[#f5f5f7]">{i18n.t.wooferDetail.changelogTitle}</h3>
    {#if changelog.length === 0}
      <p class="text-[#86868b] text-sm">{i18n.t.wooferDetail.changelogEmpty}</p>
    {:else}
      {#each changelog as entry}
        <div class="p-6 rounded-[24px] bg-white/[0.02] border border-white/[0.08] backdrop-blur-[60px]">
          <div class="flex items-center justify-between mb-3">
            <span class="text-violet-400 font-semibold">{entry.version}</span>
